...
public class ExpenseController : Controller
{
    ...
    [ProducesResponseType(typeof(ApiErrorResponses), 404)] // Not found response
    [ProducesResponseType(typeof(ApiErrorResponses), 400)] // Bad request response
    public async Task<IActionResult> Add([FromForm] Expense expense, [FromForm] ExpenseFileCreateDto fileDto)
    {
        if (!ModelState.IsValid)
        {
        ...
        {
            _logger.LogError(ex, "Error creating new expense with ID: {ExpenseId}", expense.Id);
            return StatusCode(500, $"Error creating new expense {expense.Id}");
        }
    }
