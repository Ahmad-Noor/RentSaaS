...
export class LayoutComponent  implements OnInit {
  ...
  constructor(private http: HttpClient, private master: MasterService,@Inject(DOCUMENT) private document: Document) {
    const localStorage = document.defaultView?.localStorage;
    if (localStorage) {
    const loginData = localStorage.getItem('rentSaaSLogin');
    if(loginData !=  null) {
